...
import pandas as pd
import torch
from data_free_baselines.models.model import model_pull
...
class ServerManager(BaseServer):
    def __init__(self, ct, args):
        ...
        self.full_train_dataloader = args.data_distributor.get_train_loader()
        self.full_test_dataloader = args.data_distributor.get_test_loader()
        ...
        self.local_sample_numbers = [len(args.data_distributor.get_client_train_loader(client_id).dataset)
...
class ClientManager(BaseServer):
    ...
    def finish_train_step(self, model_params, train_time):
        ...
        # logging.info(f"finish_train_step, Client_id:{self.client_id}, {self.trainer.res}")
