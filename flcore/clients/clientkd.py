...
class clientKD(Client):
    ...
    def train(self):
        ...
        W_h.to(self.device)
        # optimizer = torch.optim.SGD(model.parameters(), lr=self.sgd_lr)
        optimizer = torch.optim.Adam(model.parameters(), lr=self.adam_lr, weight_decay=self.reg)
        optimizer_g = torch.optim.SGD(global_model.parameters(), lr=self.sgd_lr)
        optimizer_W = torch.optim.SGD(W_h.parameters(), lr=self.sgd_lr)
