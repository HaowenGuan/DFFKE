...
def load_item(role, item_name, item_path=None, get_size=False):
    try:
        if not get_size:
            return torch.load(os.path.join(item_path, role + "_" + item_name + ".pt"))
        size = os.path.getsize(os.path.join(item_path, role + "_" + item_name + ".pt")) / (1024 ** 2)  # in mb
        return torch.load(os.path.join(item_path, role + "_" + item_name + ".pt")), size
    except FileNotFoundError:
        print(role, item_name, 'Not Found')
        return None

def estimate_save_size(item):
    buf = io.BytesIO()
    torch.save(item, buf)
    size_bytes = buf.tell()
