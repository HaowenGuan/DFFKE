...
class FedKTL(Server):
    ...
    def train(self):
        for i in range(self.global_rounds+1):
            ...
            if i > 1:
                download_cost = 0
                _, size = load_item('Server', 'ETF', self.save_folder_name, get_size=True)
                download_cost += size
                _, size = load_item('Server', 'data_generated', self.save_folder_name, get_size=True)
                download_cost += size
                _, size = load_item('Server', 'proj_fc', self.save_folder_name, get_size=True)
                download_cost += size
