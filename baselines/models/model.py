...
from models.resnet32x32 import resnet18_32x32
...
def model_pull(args, model_rate=1, track=False):
    ...
    elif args['model_type'] == 'ResNet18_32x32':
        if args['data_set'] in ['MNIST', 'FMNIST', 'CIFAR10', 'SVHN']:
            num_classes = 10
            ...
            raise Exception(f"Unknown data_set: {args['data_set']}")
...
class Conv(nn.Module):
    def __init__(self, hidden_size, rate, track, args):
        ...
        elif args['data_set'] in ['SVHN', 'CIFAR10', 'CIFAR100']:
            data_shape = [3, 32, 32]
            if args['data_set'] in ['SVHN', 'CIFAR10']:
                self.classes_size = 10
            elif args['data_set'] == 'CIFAR100':
...
class ResNet(nn.Module):
    def __init__(self, hidden_size, block, num_blocks, rate, track, args):
        ...
        if args['data_set'] in ['MNIST', 'FMNIST']:
            data_shape = [1, 28, 28]
            self.classes_size = 10
        elif args['data_set'] in ['CIFAR10', 'CIFAR100', 'SVHN']:
            data_shape = [3, 32, 32]
            if args['data_set'] in ['CIFAR10', 'SVHN']:
                self.classes_size = 10
            elif args['data_set'] == 'CIFAR100':
                self.classes_size = 100
        elif args['data_set'] in ['Tiny-Imagenet']:
            ...
            raise ValueError(f'Not valid data_set name {args["data_set"]}')
...
class Generator_ACGan(nn.Module):
    ...
    def __init__(self, args):
        ...
        if self.data_set in ['CIFAR10', 'FMNIST', 'MNIST', 'SVHN']:
            self.n_classes = 10
        elif self.data_set in ['CIFAR100']:
            self.n_classes = 100
