...
class DataDistributor:
    ...
    def _load_MNIST(self):
        ...
        self.train_loaders = DataLoader(dataset=train_dataset, batch_size=self.args.eval_batch_size,
                                        drop_last=drop_last_train, shuffle=True)
        self.test_loaders = DataLoader(dataset=test_dataset, batch_size=self.args.eval_batch_size,
        ...
        for client_id in range(self.client_num):
            ...
            if self.args.batch_size > 0:
                ...
                _train_dataloader = DataLoader(dataset=_train_dataset, batch_size=len(_train_dataset), shuffle=False)
                _test_dataloader = DataLoader(dataset=_test_dataset, batch_size=len(_test_dataset), shuffle=False)
            self.client_train_loaders.append(_train_dataloader)
            self.client_test_loaders.append(_test_dataloader)
    ...
    def _load_FMNIST(self):
        ...
        if not os.path.exists(f"{self.dataset_dir}/FMNIST/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth"):
            ...
            torch.save((self.train_loaders, self.test_loaders), \
            ...
            for client_id in range(self.client_num):
                ...
                self.client_train_loaders.append(_train_dataloader)
                self.client_test_loaders.append(_test_dataloader)
            torch.save((self.client_train_loaders, self.client_test_loaders, self.client_label_list, self.client_test_label_list), \
            ...
            self.train_loaders, self.test_loaders = torch.load(f"{self.dataset_dir}/FMNIST/full_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
            if self.args.data_partition_mode == 'None':
                return
            self.client_train_loaders , self.client_test_loaders, self.client_label_list, self.client_test_label_list = torch.load(f"{self.dataset_dir}/FMNIST/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
    ...
    def _load_CIFAR10(self):
        ...
        if not os.path.exists(f"{self.dataset_dir}/CIFAR-10/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth"):
            ...
            self.train_loaders, self.test_loaders = torch.load(f"{self.dataset_dir}/CIFAR-10/full_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
            if self.args.data_partition_mode == 'None':
                return
            self.client_train_loaders , self.client_test_loaders, self.client_label_list, self.client_test_label_list = torch.load(f"{self.dataset_dir}/CIFAR-10/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
    ...
    def _load_CIFAR100(self):
        ...
        if not os.path.exists(f"{self.dataset_dir}/CIFAR-100/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth"):
            ...
            if len(test_dataset.dataset.targets) % self.args.eval_batch_size == 1:
                ...
                drop_last_test = False
            self.train_loaders = DataLoader(dataset=train_dataset, batch_size=self.args.eval_batch_size, drop_last=drop_last_train, shuffle=True)
            self.test_loaders = DataLoader(dataset=test_dataset, batch_size=self.args.eval_batch_size, drop_last=drop_last_test, shuffle=True)
            torch.save((self.train_loaders, self.test_loaders), \
            ...
            self.train_loaders, self.test_loaders = torch.load(f"{self.dataset_dir}/CIFAR-100/full_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
            if self.args.data_partition_mode == 'None':
                return
            self.client_train_loaders , self.client_test_loaders, self.client_label_list, self.client_test_label_list = torch.load(f"{self.dataset_dir}/CIFAR-100/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
    ...
    def _load_SVHN(self):
        ...
        if not os.path.exists(f"{self.dataset_dir}/SVHN/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth"):
            ...
            self.train_loaders, self.test_loaders = torch.load(f"{self.dataset_dir}/SVHN/full_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
            if self.args.data_partition_mode == 'None':
                return
            self.client_train_loaders , self.client_test_loaders, self.client_label_list, self.client_test_label_list = torch.load(f"{self.dataset_dir}/SVHN/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
    ...
    def _load_FOOD101(self):
        ...
        if not os.path.exists(f"{self.dataset_dir}/Food101/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth"):
            ...
            self.train_loaders, self.test_loaders = torch.load(f"{self.dataset_dir}/Food101/full_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
            if self.args.data_partition_mode == 'None':
                return
            self.client_train_loaders , self.client_test_loaders, self.client_label_list, self.client_test_label_list = torch.load(f"{self.dataset_dir}/Food101/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
    ...
    def _load_Tiny_Imagenet(self):
        ...
        if not os.path.exists(f"{data_dir}/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth"):
            ...
            if len(testfolder) % self.args.eval_batch_size == 1:
                ...
                drop_last_test = False
            self.train_loaders = DataLoader(dataset=trainfolder, batch_size=self.args.eval_batch_size, drop_last=drop_last_train, shuffle=True)
            self.test_loaders = DataLoader(dataset=testfolder, batch_size=self.args.eval_batch_size, drop_last=drop_last_test, shuffle=True)
            ...
            self.train_loaders, self.test_loaders = torch.load(f"{data_dir}/full_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
            if self.args.data_partition_mode == 'None':
                return
            self.client_train_loaders , self.client_test_loaders, self.client_label_list, self.client_test_label_list = torch.load(f"{data_dir}/client_{self.args.data_set}_{self.args.data_partition_mode}_{self.args.non_iid_alpha}_{self.args.client_num}_{self.args.seed}.pth")
